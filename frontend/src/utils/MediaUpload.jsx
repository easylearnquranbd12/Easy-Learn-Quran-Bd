...
const uploadToCloudinary = async (file, type) => {
  ...
  try {
    ...
    Swal.fire("Upload Failed", `Could not upload ${type}. Please try again.`, "error");
    return null;
  }
};
...
const MediaUpload = ({ control, name, label, type = "image", maxSizeMB = 5 }) => {
  ...
  return (
    <div className="border p-3 rounded-md mb-2">
      <h4 className="font-medium mb-2">{label} (Max {maxSizeMB}MB)</h4>
      <Controller
        name={name}
        control={control}
        render={({ field }) => (
          <div>
            {!mediaData ? (
              <label className="relative block cursor-pointer border-2 border-dashed rounded-lg p-4 text-center hover:border-blue-400">
                ...
                <p className="text-sm text-gray-500 mt-1">Maximum size: {maxSizeMB}MB</p>
                <input
                  type="file"
                  accept={type + "/*"}
                  className="hidden"
                  ...
                  onChange={(e) => {
                    ...
                    if (file.size > maxSizeMB * 1024 * 1024) {
                      Swal.fire("File Too Large", `Maximum ${type} size is ${maxSizeMB}MB!`, "warning");
                      return;
                    }
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {mediaData.public_id ? "Uploaded to Cloudinary" : "Preview only"}
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={async () => {
                    if (mediaData.public_id) await deleteFromCloudinary(mediaData.public_id, type);
                    field.onChange(null);
                    setMediaData(null);
                  }}
