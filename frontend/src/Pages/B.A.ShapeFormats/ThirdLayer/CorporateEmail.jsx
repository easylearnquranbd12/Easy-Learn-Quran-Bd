...
const CorporateEmail = () => {
  ...
  const { data: corporateEmailFields = [], isLoading: fieldsLoading } =
    useQuery({
  ...
  const { data: corporateEmail = [], isLoading: corporateEmailLoading } =
    useQuery({
      queryKey: ["corporateEmail"],
      queryFn: async () => {
  ...
  const isLoading = fieldsLoading || corporateEmailLoading;
  useEffect(() => {
    if (corporateEmail.length > 0) {
      setOpenIndex(0);
    }
  }, [corporateEmail]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axiosPublic.post(
        "/third-layer/createExerciseCorporateEmail",
        payload,
      );
  ...
  const activeField = corporateEmailFields.find(
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-2 md:p-6 space-y-10 bg-white rounded-2xl shadow-md my-10">
      ...
      <section className="text-center">
        {corporateEmailFields.map((field) => (
