...
const InterviewsQustion = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axiosPublic.post(
        "/third-layer/createExerciseinterviewQuestions",
        payload,
      );
  ...
  const activeField = interviewQuestionsFields.find(
      ...
      {activeField && (
        <section className="card bg-white shadow-2xl rounded-2xl p-4 md:p-6 mt-10 space-y-5">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            >
              {isSubmitting ? "Adding..." : "Add Interview Questions Exercise"}
            </button>
          </form>
        </section>
      )}
...
export default InterviewsQustion;
