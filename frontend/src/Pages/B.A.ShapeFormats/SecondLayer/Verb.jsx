...
const Verb = () => {
  ...
  // Fetch verb fields
  const { data: verbFields = [], isLoading: fieldLoading } = useQuery({
    queryKey: ["verbFields"],
    queryFn: async () => {
  ...
  // Fetch verb data
  const { data: verb = [], isLoading: verbLoading } = useQuery({
    queryKey: ["verb"],
    queryFn: async () => {
  ...
  const { mutateAsync: createverbExercise } = useMutation({
    mutationFn: async (newData) => {
      const res = await axiosPublic.post(
        "/second-layer/createExerciseverb",
        newData,
      );
      return res.data;
    },
  ...
  if (fieldLoading || verbLoading) return <CustomLoading />;

  const verbField = verbFields[0] || {};
  const data = verb[0] || {};

  if (!data) return <p className="text-center mt-10">No verb data found.</p>;

  // Dynamic tab generation
  const tabs = [
    { id: "mainWord", label: verbField.mainWord },
    { id: "banglaPronunciation", label: verbField.banglaPronunciation },
    { id: "banglaMeaning", label: verbField.banglaMeaning },
    { id: "synonyms", label: verbField.synonyms },
    { id: "antonyms", label: verbField.antonyms },
    { id: "exampleEnglish", label: verbField.exampleEnglish },
    { id: "exampleBangla", label: verbField.exampleBangla },
  ];
                      ...
                      <table className="table w-full">
                        ...
                        <tbody>
                          <tr>
                            ...
                            {verbFields?.[0]?.mainWord !== "no" &&
                              verbFields?.[0]?.mainWord !== "none" && (
                                <td>
                                  <textarea
                                    {...register("mainWord")}
                            ...
                            {verbFields?.[0]?.banglaPronunciation !== "no" &&
                              verbFields?.[0]?.banglaPronunciation !==
                                "none" && (
                                <td>
                                  <textarea
                            ...
                            {verbFields?.[0]?.banglaMeaning !== "no" &&
                              verbFields?.[0]?.banglaMeaning !== "none" && (
                                <td>
                                  <textarea
                            ...
                            {verbFields?.[0]?.synonyms !== "no" &&
                              verbFields?.[0]?.synonyms !== "none" && (
                                <td>
                                  <textarea
                                    {...register("synonyms")}
                                    className="input input-base  w-72 md:w-96 min-h-20 cursor-text bg-white text-black border border-gray-300 focus:outline-none focus:ring-0 focus:border-green-300 rounded-md"
                            ...
                            {verbFields?.[0]?.antonyms !== "no" &&
                              verbFields?.[0]?.antonyms !== "none" && (
                                <td>
                                  <textarea
                                    {...register("antonyms")}
                                    className="input input-base  w-72 md:w-96 min-h-20 cursor-text bg-white text-black border border-gray-300 focus:outline-none focus:ring-0 focus:border-green-300 rounded-md"
                            ...
                            {verbFields?.[0]?.exampleEnglish !== "no" &&
                              verbFields?.[0]?.exampleEnglish !== "none" && (
                                <td>
                                  <textarea
                            ...
                            {verbFields?.[0]?.exampleBangla !== "no" &&
                              verbFields?.[0]?.exampleBangla !== "none" && (
                                <td>
                                  <textarea
