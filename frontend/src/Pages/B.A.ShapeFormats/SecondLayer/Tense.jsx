...
const Tense = () => {
  ...
  // Fetch tense fields
  const { data: tenseFields = [], isLoading: fieldLoading } = useQuery({
  ...
  // Fetch tense data
  const { data: tense = [], isLoading: tenseLoading } = useQuery({
    queryKey: ["tense"],
    queryFn: async () => {
      const res = await axiosPublic.get("/second-layer/tense");

      return res.data.data || [];
    },
  });

  const { mutateAsync: createtenseExercise } = useMutation({
    mutationFn: async (newData) => {
      const res = await axiosPublic.post(
        "/second-layer/createExercisetense",
        newData,
      );
      return res.data;
    },
    ...
    onError: (error) => {
      Swal.fire(
        "❌ Error",
        error.message || "Failed to create tense",
        "error",
      );
    },
  });

  if (fieldLoading || tenseLoading) return <CustomLoading />;

  const tenseField = tenseFields[0] || {};
  const data = tense[0] || {};

  if (!data) return <p className="text-center mt-10">No tense data found.</p>;

  // Dynamic tab generation
  const tabs = [
    { id: "mainWord", label: tenseField.mainWord },
    { id: "banglaPronunciation", label: tenseField.banglaPronunciation },
    { id: "banglaMeaning", label: tenseField.banglaMeaning },
    { id: "synonyms", label: tenseField.synonyms },
    { id: "antonyms", label: tenseField.antonyms },
    { id: "exampleEnglish", label: tenseField.exampleEnglish },
    { id: "exampleBangla", label: tenseField.exampleBangla },
  ];
                      ...
                      <table className="table w-full">
                        ...
                        <tbody>
                          <tr>
                            ...
                            {tenseFields?.[0]?.mainWord !== "no" &&
                              tenseFields?.[0]?.mainWord !== "none" && (
                                <td>
                                  <textarea
                                    {...register("mainWord")}
                            ...
                            {tenseFields?.[0]?.banglaPronunciation !== "no" &&
                              tenseFields?.[0]?.banglaPronunciation !==
                                "none" && (
                                <td>
                                  <textarea
                            ...
                            {tenseFields?.[0]?.banglaMeaning !== "no" &&
                              tenseFields?.[0]?.banglaMeaning !== "none" && (
                                <td>
                                  <textarea
                            ...
                            {tenseFields?.[0]?.synonyms !== "no" &&
                              tenseFields?.[0]?.synonyms !== "none" && (
                                <td>
                                  <textarea
                                    {...register("synonyms")}
                                    className="input input-base  w-72 md:w-96 min-h-20 cursor-text bg-white text-black border border-gray-300 focus:outline-none focus:ring-0 focus:border-green-300 rounded-md"
                            ...
                            {tenseFields?.[0]?.antonyms !== "no" &&
                              tenseFields?.[0]?.antonyms !== "none" && (
                                <td>
                                  <textarea
                                    {...register("antonyms")}
                                    className="input input-base  w-72 md:w-96 min-h-20 cursor-text bg-white text-black border border-gray-300 focus:outline-none focus:ring-0 focus:border-green-300 rounded-md"
                            ...
                            {tenseFields?.[0]?.exampleEnglish !== "no" &&
                              tenseFields?.[0]?.exampleEnglish !== "none" && (
                                <td>
                                  <textarea
                            ...
                            {tenseFields?.[0]?.exampleBangla !== "no" &&
                              tenseFields?.[0]?.exampleBangla !== "none" && (
                                <td>
                                  <textarea
