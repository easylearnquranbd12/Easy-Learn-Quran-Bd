...
const GoodMovie = () => {
  ...
  // ✅ Fetch goodMovie fields
  const { data: goodMovieFields = [], isLoading: goodMovieFieldsLoading } =
    useQuery({
      queryKey: ["goodMovieFields"],
      queryFn: async () => {
  ...
  // ✅ Fetch all goodMovies
  const { data: goodMovies = [], isLoading: goodMoviesLoading } = useQuery({
    queryKey: ["goodMovies"],
    queryFn: async () => {
  ...
  if (goodMovieFieldsLoading || goodMoviesLoading) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axiosPublic.post(
        "/fourth-layer/createExercisegoodMovie",
        payload,
      );
  ...
  const activeField = goodMovieFields.find((item) => item.isActive === "ON");

  return (
    <div className="max-w-[1400px] mx-auto  space-y-10 my-10">
      {/* ✅ goodMovie Fields */}
      <section className="text-center">
        {goodMovieFields.length === 0 ? (
          <p className="text-gray-500">No Good Movie found.</p>
        ) : (
          <div className="space-y-6">
            {goodMovieFields.map((field) => (
              <div
                key={field._id}
      ...
      <section className="max-w-[1300px] ml-auto">
        {goodMovies.length === 0 ? (
          <p className="text-gray-500 text-center">No Good Movie found.</p>
        ) : (
                  ...
                  <div className="flex justify-start items-center gap-12">
                    <div>
                      <h2 className="text-xl font-semibold text-teal-700 flex items-center gap-2">
                        <MdMovie className="text-orange-600 text-2xl" />
                        <span>{item.name}</span>
                      </h2>
                    </div>
                    <div>
      ...
      {activeField && (
        <section className="card bg-white shadow-2xl rounded-2xl p-4 md:p-6 mt-10 space-y-5">
          <h3 className="text-xl font-semibold text-teal-600">
            📖 Learning Your Good Movie Exercise
          </h3>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <MediaUpload
                control={control}
                name="ideaShareImage"
                label="Good Movie Image (Optional)"
                type="image"
                maxSizeMB={5}
            ...
            >
              {isSubmitting ? "Adding..." : "Add Good Movie Exercise"}
            </button>
          </form>
        </section>
      )}
