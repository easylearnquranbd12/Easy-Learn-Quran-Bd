...
import AdminTravelingFormatModal from "./AdminTravelingFormatModal";
import EditTravelingFormatModal from "./EditTravelingFormatModal";

const AdminTravelingFormat = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/seven-layer/traveling/${id}`, data),
    onSuccess: () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/seven-layer/traveling", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["traveling"]);
      Swal.fire(
        "✅ Success!",
...
console.log("dfcdas",traveling)
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/seven-layer/traveling/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/seven-layer/travelingField/toggle`, {
        fieldName: "isActive",
...
export default AdminTravelingFormat;
