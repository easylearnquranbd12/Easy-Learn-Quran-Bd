...
import EditgoodPoremFormatModal from "./EditgoodPoremFormatModal";
import PoremFormatModal from "./PoremFormatModal";

const AdminPoremFormat = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/seven-layer/goodPorem/${id}`, data),
    onSuccess: () => {
  ...
  const { data: goodPoremField = [], isLoading: developLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/seven-layer/goodPoremField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/seven-layer/goodPorem", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["goodPorem"]);
      Swal.fire(
        "✅ Success!",
  ...
  const { data: goodPorem = [], isLoading } = useQuery({
    queryKey: ["goodPorem"],
    queryFn: async () => {
      const res = await axiosPublic.get("/seven-layer/goodPorem");
      return res.data || [];
    },
  });
console.log("dfcdas",goodPorem)
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/seven-layer/goodPorem/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/seven-layer/goodPoremField/toggle`, {
        fieldName: "isActive",
...
export default AdminPoremFormat;
