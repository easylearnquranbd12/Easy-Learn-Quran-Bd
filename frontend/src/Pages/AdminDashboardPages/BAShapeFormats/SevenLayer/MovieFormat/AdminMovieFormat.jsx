...
import EditgoodmovieFormatModal from "./EditgoodmovieFormatModal";
import MoveFormatModal from "./MoveFormatModal";

const AdminMovieFormat = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/seven-layer/goodMovie/${id}`, data),
    onSuccess: () => {
  ...
  const { data: goodMovieField = [], isLoading: developLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/seven-layer/goodMovieField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/seven-layer/goodMovie", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["goodMovie"]);
      Swal.fire(
        "✅ Success!",
  ...
  const { data: goodMovie = [], isLoading } = useQuery({
    queryKey: ["goodMovie"],
    queryFn: async () => {
      const res = await axiosPublic.get("/seven-layer/goodMovie");
      return res.data || [];
    },
  });
console.log("dfcdas",goodMovie)
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/seven-layer/goodMovie/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/seven-layer/goodMovieField/toggle`, {
        fieldName: "isActive",
...
export default AdminMovieFormat;
