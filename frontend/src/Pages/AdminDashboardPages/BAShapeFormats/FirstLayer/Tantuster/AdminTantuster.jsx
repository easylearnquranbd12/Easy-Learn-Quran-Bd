...
const AdminTantuster = () => {
  ...
  // Create tantuster
  const { mutateAsync: createtantuster } = useMutation({
    mutationFn: async (newData) => {
      const res = await axiosPublic.post("/first-layer/tantuster", newData);
      return res.data;
    },
    onSuccess: () => {
      Swal.fire("✅ Success", "tantuster created successfully!", "success");
      reset();
  ...
  const {
    data: tantuster = [],
    isLoading: tantusterLoading,
    refetch: refetchtantuster,
    isError: tantusterError,
  } = useQuery({
    queryKey: ["tantuster"],
    queryFn: async () => {
  ...
  // delete tantuster
  const { mutateAsync: deletetantuster } = useMutation({
    mutationFn: async (id) => {
      const res = await axiosPublic.delete(`/first-layer/tantuster/${id}`);
      return res.data;
    },
    onSuccess: () => {
      Swal.fire("Deleted!", "tantuster has been deleted.", "success");
      refetchtantuster(); // Refetch the list after deletion
    },
    onError: (error) => {
      Swal.fire("Error!", "Failed to delete tantuster.", "error");
      console.error(error);
    },
  });
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You want to delete this tantuster?",
      icon: "warning",
  ...
  const visibletantuster = showAll ? tantuster : tantuster.slice(0, 10);
  // form submit
  const onSubmit = async (data) => {
    createtantuster(data);
  };
  ...
  const toggleIsActiveMutation = useMutation({
    ...
    onSuccess: (data) => {
      Swal.fire({
        icon: "success",
        title: "Success",
        text: `tantuster is now ${data.updatedValue}`,
      });
      queryClient.invalidateQueries({ queryKey: ["tantusterFields"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-2">
      <Helmet>
        <title>Quiz | tantuster</title>
      </Helmet>
      <TittleAnimation
        tittle="Create tantuster"
                ...
                {tantusterFields && tantusterFields.length > 0 && (
                  <>
                    ...
                    <div className="flex items-start justify-center gap-2 mb-2">
                      {tantusterFields[0].title || "Title"}
                      <Edit
                        onClick={() =>
                          handleEditClick(
                            "title",
                            tantusterFields[0].title,
                            tantusterFields[0].title
                          )
                        }
                    ...
                    <div className="flex items-start text-justify gap-2">
                      <span className="text-base">
                        {tantusterFields[0].description || "description"}
                      </span>
                      <Edit
                        onClick={() =>
                          handleEditClick(
                            "description",
                            tantusterFields[0].description,
                            tantusterFields[0].description
                          )
                        }
                ...
                {tantusterFields.map((item) => (
                  <div key={item._id} className="flex items-center gap-2 my-2">
                    <span className="font-semibold">
                      Create {item.title || "tantuster"} Exercise
                    </span>
                    <input
                      type="checkbox"
                      className={`toggle ${
      ...
      <div className="bg-white rounded-lg shadow-md p-5 mt-10 w-[450px] md:w-full">
        <h1 className="mb-5">
          Total tantuster Items:{" "}
          <span className="text-3xl font-bold ">{tantuster.length}</span>
        </h1>
        ...
        <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
          {tantusterLoading ? (
            ...
            <div className="p-6 text-center text-red-500">
              Error loading tantuster.
            </div>
          ) : (
            <table className="table w-full">
              {tantusterFields?.map((item, index) => (
                <thead
                  key={item._id}
              ...
              <tbody>
                {visibletantuster.length > 0 ? (
                  visibletantuster.map((row, i) => (
                    <tr
                      key={i}
