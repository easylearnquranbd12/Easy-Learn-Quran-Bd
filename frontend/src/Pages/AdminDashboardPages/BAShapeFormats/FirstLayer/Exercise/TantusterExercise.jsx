...
const TantusterExercise = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET exercise Tantuster (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-tantuster"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/first-layer/getAllExerciseTantuster",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/first-layer/deleteExerciseTantuster/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-tantuster"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  if (isError)
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load exercise Tantuster
      </p>
    );
  ...
  return (
    <div className="space-y-10">
      {visibleExercises.length > 0 ? (
        ...
        <p className="text-center py-10 text-gray-500">No Tantuster found.</p>
      )}
...
export default TantusterExercise;
