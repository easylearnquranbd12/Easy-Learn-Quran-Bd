...
const IdiomExercise = () => {
  ...
  // 🔹 GET exercise idiom (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-idiom"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/first-layer/getAllExerciseIdiom",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/first-layer/deleteExerciseIdiom/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-idiom"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  return (
    <div className="space-y-10">
      {visibleExercises.length > 0 ? (
        ...
        <p className="text-center py-10 text-gray-500">No idiom found.</p>
      )}
