...
const VocabularyExercise = () => {
  ...
  // 🔹 GET exercise vocabulary (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-vocabulary"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/first-layer/getAllExerciseVocabulary",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/first-layer/deleteExerciseVocabulary/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-vocabulary"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  if (isError)
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load exercise vocabulary
      </p>
    );
  ...
  return (
    <div className="space-y-10">
      {visibleExercises.length > 0 ? (
        ...
        <p className="text-center py-10 text-gray-500">No vocabulary found.</p>
      )}
