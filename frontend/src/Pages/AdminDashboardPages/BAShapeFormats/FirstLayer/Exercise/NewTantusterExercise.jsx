...
const NewTantusterExercise = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET exercise NewTantuster (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-NewTantuster"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/first-layer/getAllExerciseNewTantuster",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/first-layer/deleteExerciseNewTantuster/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-NewTantuster"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  if (isError)
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load exercise NewTantuster
      </p>
    );
  ...
  return (
    <div className="space-y-10">
      {visibleExercises.length > 0 ? (
        ...
        <p className="text-center py-10 text-gray-500">No NewTantuster found.</p>
      )}
...
export default NewTantusterExercise;
