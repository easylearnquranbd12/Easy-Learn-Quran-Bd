...
import SentenceFormatModal from "./SentenceFormatModal";

const AdminSentenceFormat = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/sentenceField");
  ...
  const { mutateAsync: createsentence } = useMutation({
    mutationFn: async (newData) => {
      const res = await axiosPublic.post("/six-layer/sentence", newData);
      return res.data;
    },
    onSuccess: () => {
  ...
  const {
    ...
    queryKey: ["sentence"],
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/sentence");
      return res.data.data || [];
    },
  });
  ...
  const { mutateAsync: deletesentence } = useMutation({
    mutationFn: async (id) => {
      const res = await axiosPublic.delete(`/six-layer/sentence/${id}`);
      return res.data;
    },
    onSuccess: () => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/six-layer/sentenceField/toggle`, {
...
export default AdminSentenceFormat;
