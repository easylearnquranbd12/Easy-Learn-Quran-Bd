...
import VocabularyFormatModal from "./VocabularyFormatModal";

const AdminVocabularyFormat = () => {
...
console.log(vocabularyFields)
  // Create vocabulary
  const { mutateAsync: createvocabulary } = useMutation({
    mutationFn: async (newData) => {
      const res = await axiosPublic.post("/six-layer/vocabulary", newData);
      return res.data;
    },
    onSuccess: () => {
  ...
  const { mutateAsync: deletevocabulary } = useMutation({
    mutationFn: async (id) => {
      const res = await axiosPublic.delete(`/six-layer/vocabulary/${id}`);
      return res.data;
    },
    onSuccess: () => {
  ...
  const handleEdit = (id) => {
    ...
    switch (role) {
      case "admin":
        navigate(`/admin-dashboard/edit-vocabulary-format/${id}`);
        break;

      case "moderator":
        navigate(`/moderator-dashboard/edit-vocabulary-format/${id}`);
        break;
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/six-layer/vocabularyField/toggle`, {
  ...
  return (
    <div className="max-w-[1400px] mx-auto ">
      ...
      {/* History */}
      <div className="bg-white rounded-lg shadow-md p-2 md:p-5 mt-10 w-[350px] md:w-full">
        <h1 className="mb-5">
...
export default AdminVocabularyFormat;
