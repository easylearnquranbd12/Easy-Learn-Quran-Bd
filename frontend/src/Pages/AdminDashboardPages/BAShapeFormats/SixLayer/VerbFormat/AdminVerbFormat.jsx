...
import VerbFormatModal from "./VerbFormatModal";

const AdminVerbFormat = () => {
  ...
  const { mutateAsync: createverb } = useMutation({
    mutationFn: async (newData) => {
      const res = await axiosPublic.post("/six-layer/verb", newData);
      return res.data;
    },
    onSuccess: () => {
  ...
  const {
    ...
    queryKey: ["verb"],
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/verb");
      return res.data.data || [];
    },
  });
  ...
  const { mutateAsync: deleteverb } = useMutation({
    mutationFn: async (id) => {
      const res = await axiosPublic.delete(`/six-layer/verb/${id}`);
      return res.data;
    },
    onSuccess: () => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/six-layer/verbField/toggle`, {
...
export default AdminVerbFormat;
